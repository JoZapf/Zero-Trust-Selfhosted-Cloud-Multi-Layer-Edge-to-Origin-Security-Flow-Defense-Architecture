...
flowchart TB
    ...
    subgraph CF_EDGE["Cloudflare Edge (443/tcp)"]
        ...
        CFmTLS["Client mTLS (cloud/sync)<br>Client-Cert +<br>Serial Allowlist"]
        CFWAF["WAF Rules<br>mTLS Rules, Rate Limits,<br>Geo/IP, UA"]
    ...
    subgraph ORIGIN_HOST["Origin Host (Selfhosted)"]
        subgraph ORIGIN_NET["Loopback / LAN"]
            ...
            LAN["192.168.178.1:1011<br>HTTPS (nginx, LAN<br>maintenance)"]
        end

        subgraph NGINX["nginx Reverse Proxy Container"]
            Nginx8085["listen 1011 ssl http2;<br>server_name _;<br>ssl_certificate<br>/etc/nginx/certs/cloud.crt"]
            NginxProxy["location / -> proxy_pass http://cloud-app:80;<br>X-Forwarded-*, HSTS,<br>client_max_body_size 0"]
        end
    end
